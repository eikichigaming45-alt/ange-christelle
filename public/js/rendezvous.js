...
const Rendezvous = (() => {
    ...
    function joursRestants(dt) {
        ...
        const rdvMinuit = new Date(rdvDate);
        rdvMinuit.setHours(0, 0, 0, 0);
    ...
    async function ouvrirListe(pagePasses = 0) {
        try {
            ...
            const paginationPasses = totalPages > 1 ? `
                <div class="rdv-pagination">
                    ${pagePasses > 0
                        ? `<button class="btn-rdv-secondary" onclick="Rendezvous.ouvrirListe(${pagePasses - 1})">← Précédent</button>`
                        : ''}
                    <span>${pagePasses + 1} / ${totalPages}</span>
                    ${pagePasses < totalPages - 1
                        ? `<button class="btn-rdv-secondary" onclick="Rendezvous.ouvrirListe(${pagePasses + 1})">Suivant →</button>`
                        : ''}
                </div>` : '';
                ...
                <div class="rdv-liste">
                    ...
                    ${passes.length > 0
                        ? `<div class="rdv-section-title rdv-passe-title">Passés (${passes.length})</div>
                           ${passesPaged.map(renderLigne).join('')}
                           ${paginationPasses}`
                        : ''
                    }
