...
async function chargerAdminStats() {
    ...
    if (!el) return;
    el.innerHTML = '<p style="color:#9ca3af;text-align:center">Chargement...</p>';
    try {
        ...
        el.innerHTML = `
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
                <div style="background:#eff6ff;border-radius:10px;padding:14px;text-align:center">
                    <div style="font-size:28px;font-weight:700;color:#2563eb">${d.totalUsers}</div>
                    <div style="font-size:11px;color:#6b7280;margin-top:4px">Utilisateurs</div>
                </div>
                <div style="background:#f5f3ff;border-radius:10px;padding:14px;text-align:center">
                    <div style="font-size:28px;font-weight:700;color:#7c3aed">${d.totalAdmins}</div>
                    <div style="font-size:11px;color:#6b7280;margin-top:4px">Admins</div>
                </div>
                <div style="background:#f0fdf4;border-radius:10px;padding:14px;text-align:center">
                    <div style="font-size:28px;font-weight:700;color:#16a34a">${d.profilsRemplis}</div>
                    <div style="font-size:11px;color:#6b7280;margin-top:4px">Profils remplis</div>
                </div>
                <div style="background:#fff7ed;border-radius:10px;padding:14px;text-align:center">
                    <div style="font-size:28px;font-weight:700;color:#ea580c">${d.sansProfile}</div>
    ...
    try {
        ...
        const rows = (d.users || []).map(u => `
            <div style="display:flex;align-items:center;justify-content:space-between;
                        padding:10px 0;border-bottom:1px solid #f3f4f6;gap:8px;flex-wrap:wrap">
                <div>
                    <span style="font-weight:600;font-size:14px">${u.username}</span>
                    <span style="margin-left:8px;font-size:11px;padding:2px 8px;border-radius:20px;
                                 background:${u.role === 'admin' ? '#ede9fe' : '#f3f4f6'};
                                 color:${u.role === 'admin' ? '#7c3aed' : '#6b7280'}">${u.role}</span>
                    <div style="font-size:11px;color:#9ca3af;margin-top:2px">
                        ${u.lastLogin ? new Date(u.lastLogin).toLocaleDateString('fr-FR') : 'Jamais connecté'}
                    </div>
                </div>
                <div style="display:flex;gap:6px">
                    <button onclick="adminToggleRole(${u.id},'${u.role}')"
                        style="font-size:12px;padding:5px 10px;border:1px solid #e5e7eb;
                               border-radius:6px;background:#fff;cursor:pointer">
                        ${u.role === 'admin' ? '↓ User' : '↑ Admin'}
                    </button>
                    <button onclick="adminResetPwd(${u.id},'${u.username}')"
                        style="font-size:12px;padding:5px 10px;border:1px solid #e5e7eb;
                               border-radius:6px;background:#fff;cursor:pointer">🔑</button>
                    <button onclick="adminSupprimerUser(${u.id},'${u.username}')"
                        style="font-size:12px;padding:5px 10px;border:none;
                               border-radius:6px;background:#fee2e2;color:#dc2626;cursor:pointer">🗑</button>
                </div>
            </div>
        `).join('');
...
async function adminToggleRole(id, roleActuel) {
    ...
    try {
        ...
        else document.getElementById('admin-tab-users').innerHTML =
            `<p style="color:#ef4444">${d.message}</p>`;
    } catch {
    ...
    el.innerHTML = `
        ...
        <div style="display:flex;gap:8px">
            <button onclick="adminConfirmResetPwd(${id})"
                style="flex:1;padding:10px;background:#4f46e5;color:#fff;border:none;
                       border-radius:8px;cursor:pointer;font-weight:600">Confirmer</button>
        ...
        <div id="admin-pwd-msg" style="margin-top:8px;font-size:13px;text-align:center;color:#ef4444"></div>
    `;
}
...
function adminSupprimerUser(id, username) {
    ...
    el.innerHTML = `
        <p style="font-size:14px;margin-bottom:16px">
            Supprimer définitivement <strong>${username}</strong> ?
        </p>
        <div style="display:flex;gap:8px">
            <button onclick="adminConfirmSupprimer(${id})"
                style="flex:1;padding:10px;background:#dc2626;color:#fff;border:none;
                       border-radius:8px;cursor:pointer;font-weight:600">Supprimer</button>
            <button onclick="chargerAdminUsers()"
                style="flex:1;padding:10px;background:#f3f4f6;color:#374151;border:none;
                       border-radius:8px;cursor:pointer;font-weight:600">Annuler</button>
        </div>
